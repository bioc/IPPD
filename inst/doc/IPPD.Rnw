...
\author{
        \begin{tabular}{ll}
        Martin Slawski $^{1}$ & \url{ms@cs.uni-sb.de}  \\
        Rene Hussong $^2$ & \url{rene@bioinf.uni-sb.de} \\
        Matthias Hein $^1$ & \url{hein@cs.uni-sb.de}
      \end{tabular} 
    }
...
options(prompt = "R> ", continue = " ", warn = -1, width = 80)                                       
...
plot(x[x >= 804 & x <= 807], y[x >= 804 & x <= 807], xlab = "x: 804 <= x <= 807", 
...
show(fitEMG)
mse.EMG <- data.frame(mse = slot(fitEMG,"peakfitresults")[,"rss"] / slot(fitEMG,"peakfitresults")[,"datapoints"], 
                      peakshape = rep("EMG", nrow( slot(fitEMG,"peakfitresults"))))
mse.Gauss <- data.frame(mse = slot(fitGauss,"peakfitresults")[,"rss"] / slot(fitGauss,"peakfitresults")[,"datapoints"], 
...
visualize(fitEMG, type = "model", modelfit = TRUE, parameters = c("sigma", "alpha"), cex.lab = 1.5, cex.axis = 1.25)
...
ratio bigger than three are maintained. It has the following form.
